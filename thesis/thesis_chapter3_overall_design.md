...
**表3.10 disease_trend表（疾病趋势资讯表）**

| 字段名 | 类型 | 说明 |
|:---|:---|:---|
| id | BIGINT (PK) | 主键 |
| user_id | BIGINT (FK) | 关联用户 |
| diseaseName | VARCHAR(200) | 疾病名称（如"diabetes"） |
| diseaseCode | VARCHAR(20) | 标准疾病代码（如"DM"） |
| riskLevel | VARCHAR(10) | 风险等级（LOW/MEDIUM/HIGH） |
| advisoryType | VARCHAR(20) | 资讯类型（NEWS/VACCINE/OUTBREAK） |
| source | VARCHAR(100) | 数据源（如"NEWS_API"） |
| sourceUrl | VARCHAR(1000) | 原文链接 |
| advisoryText | TEXT | 资讯标题/摘要内容 |
