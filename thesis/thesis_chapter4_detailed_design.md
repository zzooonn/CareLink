...
系统在前端模拟生成12导联ECG数据：每条导联以正弦基波叠加QRS复合波模板的方式生成**1000个采样点**的时域信号（等效于100Hz采样率下的10秒信号），支持"正常"与"带噪声"两种模式。生成的数据以`number[][]`格式（12×1000）组织。

> **采样率适配说明：** AI模型训练时使用的PTB-XL数据集采样率为500Hz（10秒×500Hz=5000采样点），而移动端模拟生成的ECG为1000采样点（等效100Hz）。为解决这一不一致，后端`EcgAnalysisService`在转发数据至AI推理服务时，将采样率参数`fs`设置为100（而非500）。AI推理服务端（FastAPI）接收到`fs=100`的请求后，通过`scipy.signal.resample_poly`将1000采样点上采样至5000采样点（5×重采样），再送入模型推理，确保与训练时的信号长度与频率特征一致。

这一采样率适配设计使移动端能够使用较低计算开销的模拟数据，同时保证AI模型接收到格式规范的标准输入。
...
**类别分布与不均衡性：** PTB-XL数据集存在显著的类别不均衡问题，各类别样本量如表4.2所示。由于允许多标签同时标注，各类别计数之和超过总记录数。

**表4.2 PTB-XL数据集类别分布（全集21,837条）**

| 类别 | 临床含义 | 正样本数 | 占比 | 不均衡程度 |
|:---|:---|:---:|:---:|:---|
| NORM | 正常心电图 | 9,528 | 43.6% | 多数类 |
| STTC | ST段/T波改变 | 5,250 | 24.0% | 中等 |
| MI | 心肌梗死 | 5,469 | 25.0% | 中等 |
| CD | 传导障碍 | 4,907 | 22.5% | 中等 |
| HYP | 心室肥厚 | 2,655 | 12.2% | **少数类（最不均衡）** |

> 数据来源：Wagner et al. (2020) PTB-XL论文[8]；"占比"为各类正样本数/总记录数，各列之和超过100%反映多标签特性。

HYP类别正样本仅占12.2%，约为NORM类的1/3.6，是导致HYP分类性能最低的核心原因。这一不均衡特性直接驱动了本课题采用WeightedSampler（对HYP正样本赋权4倍）与非对称损失（ASL，对负样本梯度惩罚更大）两项针对性技术手段。
